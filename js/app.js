...
function addTask(fromList){
    ...
    function checkTask() {
        ...
        checkBtn.className === "far fa-circle" ? lists[id].completed = true : lists[id].completed = false;
        saveValue(checkBtn);

        function saveValue(el){
            const dataLS = JSON.parse(localStorage.getItem('item.list'));
            (fromList) ? dataLS[id].completed =  fromList.completed :  dataLS[id].completed = lists[id].completed;
